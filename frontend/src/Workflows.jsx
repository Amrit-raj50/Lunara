...
export function TemplateBuilderView() {
  ...
  const saveTemplate = async () => {
    ...
    try {
      ...
      fetch('http://localhost:8000/api/templates').then(r => r.json()).then(d => setSavedTemplates(d.templates || [])).catch(() => {});
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      for (let t of updatedTracks) {
        for (let c of t.clips) {
          if (c._localFile) {
            ...
            const res = await fetch('http://localhost:8000/api/upload_clip', { method: 'POST', body: fd });
      ...
      // 2. Request preview
      const res = await fetch('http://localhost:8000/api/render/preview', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
...
export function BatchRenderView() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => fetch(`http://localhost:8000/api/render/${jobId}`).then(r => r.json()).then(setJobStatus), 1000);
  ...
  const startRender = async () => {
    ...
    const r = await fetch('http://localhost:8000/api/render/batch', { method: 'POST', body: fd });
                      ...
                      {!res ? <span style={{ color: '#4a5568' }}>Pending</span>
                        : res.success ? (
                          <>
                            ...
                            <a href={`http://localhost:8000/api/render/output/${res.output_name}`} download style={{ color: '#7c6af7', textDecoration: 'none', fontSize: '0.75rem', fontWeight: 700, border: '1px solid #7c6af7', padding: '2px 8px', borderRadius: 4 }}>Download</a>
                          </>
                        )
